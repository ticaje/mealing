 ...
 *
 */
class Recomiendo_Recipes_Controller_Adminhtml_Codifier_BaseController extends Mage_Adminhtml_Controller_Action
{

  protected $_activeMenu = "recipes";
   ...
   * Init actions
   *
   * @return Recomiendo_Recipes_Controller_Adminhtml_Codifier_BaseController
   */
  protected function _initAction()
  {
    ...
    $this->loadLayout()
      ...
      ->_addBreadcrumb(
        Mage::helper('recomiendo_recipes')->__($this->_currentEntity),
        Mage::helper('recomiendo_recipes')->__($this->_currentEntity)
      )
      ->_addBreadcrumb(
        Mage::helper('recomiendo_recipes')->__('Gestión '.$this->_currentEntity),
        Mage::helper('recomiendo_recipes')->__('Gestión '.$this->_currentEntity)
      )
      ;
    return $this;
    ...
    if ($entityId) {
      ...
      if (!$model->getId()) {
        $this->_getSession()->addError(
          Mage::helper('recomiendo_recipes')->__('La entidad %s no existe', $this->_titleLabel)
        );
      ...
      $this->_title(Mage::helper('recomiendo_recipes')->__('Nuevo '. $this->_titleLabel));
    }
  ...
  public function deleteAction()
  {
    ...
    if ($itemId) {
      try {
        ...
        $model->load($itemId);
        if (!$model->getId()) {
          Mage::throwException(Mage::helper('recomiendo_recipes')->__('No se pudo encontrar la entidad %s', $this->_titleLabel));
        }
        $model->delete();

        // display success message
        $this->_getSession()->addSuccess(
          Mage::helper('recomiendo_recipes')->__('La entidad %s ha sido eliminada', $this->_titleLabel)
        );
      } catch (Mage_Core_Exception $e) {
        $this->_getSession()->addError($e->getMessage());
      } catch (Exception $e) {
